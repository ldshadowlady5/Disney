...
public class BlockDisneyPlanksStairs extends Block{
	...
	public BlockDisneyPlanksStairs(IBlockState modelState)
	{
		super(Material.WOOD);
	...
	public boolean doesSideBlockRendering(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing face)
	{
		...
		BlockStairs.EnumHalf half = state.getValue(HALF);
		EnumFacing side = state.getValue(FACING);
		BlockStairs.EnumShape shape = state.getValue(SHAPE);
		if (face == EnumFacing.UP) return half == BlockStairs.EnumHalf.TOP;
		if (face == EnumFacing.DOWN) return half == BlockStairs.EnumHalf.BOTTOM;
		if (shape == BlockStairs.EnumShape.OUTER_LEFT || shape == BlockStairs.EnumShape.OUTER_RIGHT) return false;
		if (face == side) return true;
		if (shape == BlockStairs.EnumShape.INNER_LEFT && face.rotateY() == side) return true;
		if (shape == BlockStairs.EnumShape.INNER_RIGHT && face.rotateYCCW() == side) return true;
		return false;
	}
